...
This file mirrors `CLAUDE.md` so Codex/OpenAI agents follow the same starter contract. Keep `AGENTS.md` and `CLAUDE.md` in sync.

## 1. Hard Constraints

- Job: build pages and app features by composing the shipped system.
- Start each task in this order: `AI-PLAYBOOK.md` -> `src/config/component-registry.json` -> `COMPOSITION-RECIPES.md` -> relevant `.claude/rules/*` -> nearest local analog.
