...
function generateFontsScss(): string {
  ...
  scss += `/* Source: src/config/design-tokens.ts */\n`;
...
function generateFontRegistry(): string {
  ...
  ts += ` * Source: src/config/design-tokens.ts\n`;
  ts += ` *\n`;
...
async function main() {
  try {
    ...
    writeIfChanged(PATHS.scss, scssContent);
    ...
    writeIfChanged(PATHS.registryJson, `${JSON.stringify(registry, null, 2)}\n`);
    console.log(`   └─ ⚙️  Registry: src/config/void-registry.json`);

    // Generate physics JSON.
    writeIfChanged(PATHS.physicsJson, `${JSON.stringify(VOID_TOKENS.physics, null, 2)}\n`);
